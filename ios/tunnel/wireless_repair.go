...
// already trusted for RemotePairing using a USB-attached device. It uses the
// same reliable USB-RSD route as RepairRemotePairingOverUSB but never starts
// manual pairing and never prompts the user for RemotePairing consent.
...
// device. The same PairRecordManager used by ManualPairWithAddr must be supplied
// so the repaired host identity is later used for WiFi pair verification.
